...
Distinguished = Redis:sismember(MILAN.."Distinguished:Group"..ChatId,UserId)
StatusMember = LuaTele.getChatMember(ChatId,UserId).status.luatele
if tonumber(UserId) == 1918968211 then
Status = true
elseif tonumber(UserId) ==  1498753532 then
Status = true
elseif UserId == Sudo_Id then  
Status = true
elseif UserId == MILAN then
...
keyboardd.inline_keyboard = {
{
{text = '< Developers MILAN  >', url = "t.me/C_T_V"}
},
}
local msg_id = msg.id/2097152/0.5 
 https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
Redis:srem(BLANK.."ChekBotAdd",'-100'..data.supergroup.id)
