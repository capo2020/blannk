...
local N = (Redis:get(MILAN.."Name:Bot") or "بلانك")
...
if text == 'رفع نسخه بلانك' and msg.reply_to_message_id ~= 0 then
if not msg.ControllerBot then 
return LuaTele.sendText(msg_chat_id,msg_id,'\n*☆︙ هاذا الامر يخص { '..Controller_Num(1)..' }* ',"md",true)  
end
local Message_Reply = LuaTele.getMessage(msg.chat_id, msg.reply_to_message_id)
...
return LuaTele.sendText(msg_chat_id,msg_id,'☆︙ تم استرجاع المجموعات من نسخه بلانك')
else
...
local Namebot = (Redis:get(MILAN.."Name:Bot") or "بلانك")
local BotName = {
...
if text == (Redis:get(MILAN.."Name:Bot") or "بلانك") then
local photo = LuaTele.getUserProfilePhotos(MILAN)
local ban = LuaTele.getUser(MILAN)
local Namebot = (Redis:get(MILAN.."Name:Bot") or "بلانك")
local BotName = {
...
local CmdStart = '*\n☆︙ أهلآ بك في بوت '..(Redis:get(MILAN.."Name:Bot") or "بلانك")..
...
local TextHelp = [[*
• أهلا بك في قائمة العاب سورس بلانك اختر نوع الالعاب 
*]]
LuaTele.editMessageText(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
end
...
local TextHelp = [[*
• مرحبا بك في الالعاب المتطورة الخاص بسورس بلانك 
